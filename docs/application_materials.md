...
GeminiはObserve-Reason-Decideの流れで状況を判断し、命令や診断ではなく、自然な敬語でそっと声をかけます。さらに不確かな点を家族・薬剤師へ渡せるコピー可能なメモに変換します。SilverLinkは、AIを「使う道具」から「関係性を静かに守る層」へ変える、日本の社会福祉DXのための提案です。
...
| Cognitive | Gemini stable API + Gemini Live experimental probe | Stable `generateContent` path for demo, Live probe behind a settings toggle |
| Interaction | Flutter Zero-UI + Google Chirp 3 HD TTS | Pulsing ambient sphere, warm natural Japanese voice, medicine card, copyable handoff memo, hidden debug injector |
...
6. Add structured medicine cards and copyable family/pharmacist handoff memos to show responsible escalation.
